...
export const NavBar: React.FC<NavBarProps> = ({ user, cartCount, onNavigate, onLogin, onLogout, isDarkMode, toggleTheme, storeConfig, currentView, adminActiveTab }) => {
  ...
  return (
    <nav className="h-16 w-full border-b border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md sticky top-0 z-50 px-4 md:px-8 flex items-center justify-between transition-all duration-300" aria-label="Navegación principal">
      <div className="flex items-center gap-4 md:gap-8">
        <div onClick={() => handleNav('LANDING')} className="cursor-pointer flex items-center gap-4 group">
           {storeConfig.logoUrl ? (
               ...
               <div className="w-10 h-10 md:w-12 md:h-12 bg-zinc-900 dark:bg-white flex items-center justify-center font-black text-white dark:text-zinc-900 text-sm md:text-base rounded-xl shadow-lg">LM</div>
           )}
           <span 
        ...
        {/* Page Title - Between logo and menu */}
        {getPageTitle() && (
          <div className="hidden md:flex items-center">
            <div className="h-8 w-px bg-zinc-300 dark:bg-zinc-700 mx-5" />
            <span className="text-base md:text-lg font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-tight">
              {getPageTitle()}
            </span>
          </div>
        )}
        ...
        {user && (
          <div className="hidden md:flex items-center gap-2 bg-zinc-100/50 dark:bg-zinc-800/50 p-1.5 rounded-full border border-zinc-200 dark:border-zinc-700">
             ...
             <button type="button" onClick={() => handleNav('CUSTOMIZER')} aria-label="Abrir personalizador" className="px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-all duration-200 shadow-sm hover:shadow-md flex items-center gap-2 active:scale-95">
      ...
      <div className="flex items-center gap-3 md:gap-5">
        ...
        <button onClick={toggleTheme} aria-label="Cambiar tema" className="hidden md:flex items-center justify-center text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-all duration-200 w-10 h-10 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 active:scale-95">
        ...
        {user ? (
           <div className="hidden md:flex items-center gap-4 pl-6 border-l border-zinc-200 dark:border-zinc-700">
             {user.role === UserRole.ADMIN ? (
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="bg-zinc-900 text-white border border-transparent px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-zinc-800 hover:shadow-lg transition-all duration-200 flex items-center gap-2 active:scale-95"
                  >
                    <Home size={14}/> Admin
                  </button>
                  <button 
             ...
             <div className="flex items-center gap-2 cursor-pointer group relative">
                 <img src={user.avatarUrl} className="w-11 h-11 rounded-full border-2 border-zinc-200 dark:border-zinc-700 group-hover:border-zinc-400 dark:group-hover:border-zinc-500 transition-colors object-cover" alt={`Avatar de ${user.name}`}/>
           ...
           <button onClick={onLogin} aria-label="Iniciar sesión" className="hidden md:flex text-[10px] font-black uppercase tracking-widest text-zinc-900 dark:text-white bg-zinc-100 dark:bg-zinc-800 items-center gap-2 transition-all duration-200 px-6 py-3 hover:bg-zinc-900 hover:text-white dark:hover:bg-white dark:hover:text-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 hover:border-transparent active:scale-95">
      ...
      {/* Mobile Menu Overlay */}
      {isMenuOpen && (
          <div className="fixed inset-0 top-0 w-full h-screen bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md z-50 flex flex-col p-10 pt-32 gap-8 animate-in slide-in-from-right duration-300 md:hidden" role="dialog" aria-modal="true" aria-label="Menú de navegación">
              {user ? (
                  <>
                    <button onClick={() => handleNav('SHOP')} className="text-left text-4xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white border-b-2 border-zinc-200 dark:border-zinc-800 pb-6 hover:pl-4 transition-all flex items-center gap-4">
                      <List size={24} /> Catálogo
                    </button>
                    <button onClick={() => handleNav('FONTS_SHOWCASE')} className="text-left text-4xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white border-b-2 border-zinc-200 dark:border-zinc-800 pb-6 hover:pl-4 transition-all flex items-center gap-4">
                      <Palette size={32} /> Fuentes
                    </button>
                    <button onClick={() => handleNav('CUSTOMIZER')} className="text-left text-4xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white border-b-2 border-zinc-200 dark:border-zinc-800 pb-6 hover:pl-4 transition-all flex items-center gap-4">
                      <PenTool size={24} /> Personalizar
                    </button>
                    {user.role === UserRole.ADMIN ? (
                      <>
                        <button onClick={() => handleNav('ADMIN_DASHBOARD')} className="text-left text-4xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white border-b-2 border-zinc-200 dark:border-zinc-800 pb-6 hover:pl-4 transition-all flex items-center gap-4">
                          <Home size={32} /> Panel Admin
                        </button>
                        <button onClick={() => handleNav('CLIENT_DASHBOARD')} className="text-left text-4xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white border-b-2 border-zinc-200 dark:border-zinc-800 pb-6 hover:pl-4 transition-all flex items-center gap-4">
                  ...
                  <button onClick={() => { onLogin(); setIsMenuOpen(false); }} className="text-left text-4xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white border-b-2 border-zinc-200 dark:border-zinc-800 pb-6 hover:pl-4 transition-all flex items-center gap-4">
