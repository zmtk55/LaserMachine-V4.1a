...
export const ClientDashboard: React.FC<ClientDashboardProps> = ({
  ...
  // Tab transition wrapper - key ensures re-animation on every tab change
  ...
  const renderHome = () => (
    <TabContent>
      <div className="space-y-6">
        {/* Welcome Hero Card */}
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-zinc-900 via-zinc-800 to-black p-6 text-white shadow-xl">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                ...
                <h2 className="font-bold text-xl text-white">¡Hola, {user.name.split(' ')[0]}!</h2>
              </div>
            </div>
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-500/20 via-amber-400/10 to-transparent border border-amber-500/30 p-5 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-amber-400/90 mb-1 flex items-center gap-1.5 font-bold">
                    <Award size={14} />
                    Tus puntos acumulados
                  </p>
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-black text-amber-400 tracking-tight">{formattedPoints}</span>
                    <span className="text-sm text-amber-400/80 font-bold">pts</span>
                  </div>
                  <p className="text-xs text-white/60 mt-1.5 font-medium">
                    Equivalente a <span className="text-white font-bold">{pointsValue}</span> de descuento
                  </p>
                </div>
                <div className="w-14 h-14 bg-gradient-to-br from-amber-400 to-amber-500 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-500/20">
              ...
              <div className="flex gap-3 mt-4">
                <button
                  ...
                  className="flex-1 bg-amber-400 hover:bg-amber-300 text-black text-xs font-bold py-2.5 px-4 rounded-xl transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20"
                >
                  <Wallet size={14} /> Canjear
                </button>
                <button
                  onClick={() => document.getElementById('points-history')?.scrollIntoView({ behavior: 'smooth' })}
                  className="flex-1 bg-white/10 hover:bg-white/20 text-white text-xs font-bold py-2.5 px-4 rounded-xl transition-all duration-200 active:scale-95 flex items-center justify-center gap-2"
                >
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { icon: Package, value: stats.totalOrders, label: 'Pedidos', color: 'text-sky-500', bg: 'bg-sky-500/10' },
            ...
            <button
              ...
              className="group bg-white dark:bg-zinc-900/80 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800 text-center transition-all duration-200 hover:shadow-lg hover:border-zinc-300 dark:hover:border-zinc-700 active:scale-95"
            >
        ...
        {banners.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500 font-bold">{banners.length} destacados</span>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-3 snap-x scrollbar-hide -mx-4 px-4">
              {banners.map((banner) => (
                ...
                >
                  <div className="relative h-40 rounded-2xl overflow-hidden shadow-lg group cursor-pointer transition-all duration-200 hover:shadow-xl">
                    <img
                      src={banner.image}
                      alt={banner.title}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      ...
                      {banner.subtitle && (
                        <p className="text-xs text-zinc-300 font-medium">{banner.subtitle}</p>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {myOrders.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setActiveTab('orders')}
                className="text-xs text-amber-600 dark:text-amber-400 font-bold flex items-center gap-1 hover:gap-2 transition-all duration-200 active:scale-95"
              >
              ...
              {myOrders.slice(0, 2).map((order) => {
                ...
                return (
                  <button
                    ...
                    className="group w-full text-left bg-white dark:bg-zinc-900/80 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800 transition-all duration-200 hover:shadow-lg hover:border-zinc-300 dark:hover:border-zinc-700 active:scale-[0.98]"
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 ${status.bg} ${status.border} border rounded-xl flex items-center justify-center transition-transform duration-200 group-hover:scale-110`}>
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-zinc-500 font-medium">{formatShortDate(order.createdAt)}</span>
                        </div>
                      </div>
        ...
        {promotions.length > 0 && (
          <div className="space-y-3">
            ...
            {promotions.slice(0, 2).map((promo) => (
              <div
                key={promo.id}
                className="group bg-white dark:bg-zinc-900/80 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 transition-all duration-200 hover:shadow-lg hover:border-zinc-300 dark:hover:border-zinc-700"
              >
                <div className="flex">
                  ...
                  <div className="flex-1 p-4 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 font-medium line-clamp-1">{promo.description}</p>
                    </div>
                    <div className="flex items-center justify-between mt-2">
                      ...
                      {promo.code && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-xl text-xs font-bold text-zinc-700 dark:text-zinc-300 flex items-center gap-1.5 transition-all duration-200 active:scale-95"
                        >
        ...
        {recentTransactions.length > 0 && (
          <div id="points-history" className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 font-medium">{formatShortDate(transaction.createdAt)}</p>
                    </div>
                  </div>
            ...
            {recentTransactions.length > 5 && (
              <button className="w-full text-center text-xs text-amber-600 dark:text-amber-400 font-bold py-2 hover:underline transition-all duration-200 active:scale-95">
        ...
        <button
          ...
          className="w-full py-4 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-all duration-200 shadow-lg shadow-emerald-500/20 active:scale-95"
        >
  ...
  const renderOrders = () => {
    const renderOrderCard = (order: Order) => {
      ...
      return (
        <div
          key={order.id}
          className="bg-white dark:bg-zinc-900/80 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden transition-all duration-200 hover:shadow-lg hover:border-zinc-300 dark:hover:border-zinc-700"
        >
          {/* Order Header */}
          <button
            onClick={() => setExpandedOrder(isExpanded ? null : order.id)}
            className="w-full text-left p-4 cursor-pointer transition-colors active:scale-[0.99]"
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-11 h-11 ${status.bg} ${status.border} border rounded-xl flex items-center justify-center transition-transform duration-200`}>
            ...
            {order.estimatedDeliveryDate && (
              <div className="mt-3 flex items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-950/50 px-3 py-2 rounded-xl w-fit font-medium">
                <Truck size={14} className="text-zinc-400" />
                <span>Entrega estimada: <span className="font-bold text-zinc-900 dark:text-white">{formatDate(order.estimatedDeliveryDate)}</span></span>
              </div>
            )}
          </button>
                  ...
                  return (
                    <div key={itemIdx} className="flex gap-3 bg-zinc-50 dark:bg-zinc-950/50 p-3 rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-zinc-500 font-medium">{item.colorName} • Cantidad: {item.quantity}</p>
              ...
              <div className="flex gap-2 pt-2">
                {needsApproval && (
                  <button
                    ...
                    className="flex-1 py-3 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all duration-200 shadow-lg shadow-purple-500/20 active:scale-95 animate-pulse"
                  >
                    <Sparkles size={14} />
                    Aprobar diseño
                  </button>
                ...
                <button
                  onClick={() => setSelectedOrder(order)}
                  className="flex-1 py-3 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all duration-200 active:scale-95"
                >
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all duration-200 shadow-lg shadow-emerald-500/20 active:scale-95"
                >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500 font-medium">{myOrders.length} pedido{myOrders.length !== 1 ? 's' : ''} en total</p>
            </div>
            ...
            <EmptyState
              ...
              action={
                <button
                  ...
                  className="px-6 py-2.5 bg-amber-400 hover:bg-amber-300 text-black rounded-xl text-sm font-bold transition-all duration-200 active:scale-95 shadow-lg shadow-amber-500/20"
                >
                  Explorar catálogo
                </button>
              }
              ...
              {completedOrdersList.length > 0 && (
                <div className="pt-2 border-t border-zinc-200 dark:border-zinc-800">
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 px-4 py-3 text-left transition-all duration-200 hover:bg-zinc-100 dark:hover:bg-zinc-800/80 active:scale-[0.99]"
                  >
      ...
      <TabContent>
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-medium">{activeProducts.length} productos disponibles</p>
          </div>
              ...
              {activeProducts.map((product) => (
                ...
                >
                  <div className="relative aspect-square bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
                    ...
                    <button className="absolute top-2 right-2 p-2 bg-black/20 hover:bg-black/40 backdrop-blur-sm rounded-full text-white opacity-0 group-hover:opacity-100 transition-all duration-300 active:scale-95">
                      <Heart size={14} />
                    </button>
                  </div>
                  <div className="p-3">
                    <p className="text-[10px] text-zinc-500 uppercase tracking-wide font-bold">{product.brand}</p>
      ...
      <TabContent>
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-medium">{activeCoupons.length} cupones disponibles</p>
          </div>
          ...
          {activeCoupons.length === 0 ? (
            ...
            <div className="space-y-4">
              {activeCoupons.map((coupon) => (
                <div
                  key={coupon.id}
                  className="bg-white dark:bg-zinc-900/80 rounded-2xl p-5 border border-zinc-200 dark:border-zinc-800 relative overflow-hidden transition-all duration-200 hover:shadow-lg hover:border-zinc-300 dark:hover:border-zinc-700"
                >
                  ...
                  <div className="relative">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-amber-400 hover:bg-amber-300 text-black rounded-xl text-xs font-bold transition-all duration-200 active:scale-95 flex items-center gap-2 shadow-lg shadow-amber-500/20"
                      >
                    ...
                    <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800">
                      <code className="inline-block bg-zinc-100 dark:bg-zinc-800 px-4 py-2 rounded-xl font-mono text-lg font-bold text-zinc-900 dark:text-white tracking-wider">
                        {coupon.code}
                      </code>
                    </div>
          ...
          {expiredCoupons.length > 0 && (
            <div className="mt-8">
              <p className="text-sm text-zinc-500 mb-3 font-bold">Cupones expirados</p>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-zinc-500 font-bold">{coupon.code}</span>
                      </div>
                      <span className="text-xs text-zinc-400 font-bold">Expirado</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white dark:bg-zinc-900/80 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800 space-y-4">
            <div className="relative">
              ...
              <input
                ...
                maxLength={20}
                className="w-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 py-4 pl-14 pr-4 rounded-xl text-xl font-bold text-zinc-900 dark:text-white outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 placeholder:text-zinc-400 transition-all duration-200"
              />
            </div>

            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-3 flex-1 min-w-[200px]">
                <span className="text-sm text-zinc-500 whitespace-nowrap font-bold w-12">{fontSize}px</span>
                <input
                  type="range"
                  min={20}
                  max={64}
              ...
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="w-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 py-2.5 pl-10 pr-4 rounded-xl text-sm font-bold text-zinc-900 dark:text-white outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 placeholder:text-zinc-400 transition-all duration-200"
                />
              </div>
            </div>
          </div>
            ...
            {filteredFonts.map((font) => {
              ...
              return (
                <button
                  ...
                  className={`relative p-5 rounded-2xl border-2 text-left transition-all duration-200 active:scale-[0.98] ${
                    isSelected
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-zinc-400 dark:text-zinc-500 font-mono mb-0.5 font-bold">
                        #{font.id}
                      </p>
                      <p className={`font-bold text-sm ${font.cssFamily}`}>
                        {font.name}
                      </p>
                  ...
                  {isSelected && onSelectFontForCustomizer && (
                    <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                      <button
                        ...
                        className="w-full py-3 px-4 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20"
                      >
                        <ArrowRight size={18} />
          ...
          <div className="min-h-full p-4 flex items-start justify-center">
            ...
            >
              <button
                ...
                className="absolute top-4 right-4 p-2.5 bg-zinc-100 dark:bg-zinc-900 rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-all duration-200 active:scale-95"
                aria-label="Cerrar"
              >
                <X size={20} />
              </button>
              ...
              <div className={`${s.bg} ${s.border} border rounded-2xl p-4 mb-6`}>
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 ${s.color} rounded-xl flex items-center justify-center shadow-md ${s.shadow}`}>
                    <s.icon size={24} className="text-white" />
                  </div>
                  <div>
                    <p className={`font-bold ${s.text}`}>{s.label}</p>
                    <p className="text-xs text-zinc-500 font-medium">{s.description}</p>
                  </div>
                </div>
              </div>
                ...
                {selectedOrder.items.map((item, idx) => (
                  <div key={idx} className="border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                    <div className="bg-zinc-50 dark:bg-zinc-900 px-4 py-3 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                      ...
                      <span className="text-xs text-zinc-500 font-medium">{item.colorName}</span>
                    </div>
              ...
              <div className="mt-6 pt-6 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
                ...
                {selectedOrder.discountAmount ? (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-zinc-500 font-medium">Descuento</span>
                    <span className="text-emerald-600 font-bold">-{formatCurrency(selectedOrder.discountAmount)}</span>
                  </div>
                ) : null}
                {selectedOrder.pointsRedeemed ? (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-zinc-500 font-medium">Puntos canjeados</span>
                    <span className="text-amber-600 font-bold">-{formatCurrency(selectedOrder.pointsRedeemed)}</span>
                  </div>
                ) : null}
              ...
              <div className="mt-6 pt-6 border-t border-zinc-200 dark:border-zinc-800 flex gap-3">
                <button
                  ...
                  className="flex-1 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all duration-200 shadow-lg shadow-emerald-500/20 active:scale-95"
                >
                ...
                {selectedOrder.status === OrderStatus.COMPLETED && (
                  <button
                    ...
                    className="flex-1 py-3.5 bg-amber-400 hover:bg-amber-300 text-black rounded-xl font-bold flex items-center justify-center gap-2 transition-all duration-200 shadow-lg shadow-amber-500/20 active:scale-95"
                  >
                    <RefreshCcw size={18} />
                    Reordenar
                  </button>
  ...
  return (
    <div className="min-h-full bg-zinc-50 dark:bg-black pb-28">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-500 font-bold">Panel de cliente</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {onBackToAdmin && (
                <button
                  ...
                  className="hidden sm:flex px-3 py-2 text-xs font-bold text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors active:scale-95"
                >
                  ← Volver a admin
                </button>
              )}
              <button
                ...
                className="p-2.5 bg-zinc-100 dark:bg-zinc-900 rounded-xl text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-all duration-200 active:scale-95"
      ...
      {hasPendingApprovals && (
        <div className="sticky top-[57px] left-0 right-0 z-30 bg-gradient-to-r from-purple-500 to-purple-600 text-white px-4 py-3 shadow-lg animate-in slide-in-from-top duration-300">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-xl transition-all duration-200 active:scale-95"
            >
              Ver ahora →
            </button>
          </div>
